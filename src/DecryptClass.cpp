...
private:
    ...
    */
    ...
    /**
        Exit the program & print warning
    */
    void terminate();
...
public:
    ...
    */
    void decrypt(string& cyphered, string pass, char* inFileName, char* outFileName, bool RemoveOrigin, bool b64, bool pDisplacement, bool vectorAlgorithm, bool loud,bool header,bool pGUI);
    /**
...
void Decryptor::decrypt(string& text, string pass, char* inFileName, char* outFileName, bool RemoveOrigin, bool b64, bool pDisplacement,bool vectorAlgorithm, bool loud,bool header,bool pGUI) {

    flow = text;
    //cout << "FLOW RECIEVED: " << flow << endl;
	...
	//......................................................
	//OBSOLETE: THIS IS GONNA BE CHANGED TO RETURN A STRING.
    //6. OUTPUT TO TEXTFILE
    //output(outFileName);
	//......................................................
	text = cylines;
    //cout << "TEXT RETURN: " << text << endl;
    ...
    if (RemoveOrigin) {
        if (remove(inFileName) != 0) {
            cout << "Couldn't delete original file";
        }
    }
    */
...
void Decryptor::vector_Algorithm() {
    ...
    //Precalculations
    if (password.length() >= cylines.length()){
        return;
    }
    ...
    for (unsigned i= 0; i < cylines.length(); i+=(PocketSize)) {
        cyVector.push_back(cylines.substr(i,PocketSize));
    }

    //Enforcing Order (Decryption)

    for (unsigned i=0; i<(cyVector.size()-2); i+=2) { //(VectorSize-1 = Final Packet.) -1 Second to last packet
    ...
    cylines = "";
    for (unsigned i= 0; i<cyVector.size(); i++) {
...
void Decryptor::terminate() {
    cout << "\nERROR: Wrong Password or Wrong Selected Options Given\n"
         << "The text you are trying to decrypt was not cyphered with\n"
         << "the given password or those selected options.\n";
    exit(0);
}
